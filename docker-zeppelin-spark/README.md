...
Data downloaded from the page:
https://data.world/jrm/traffic-violations

docker downloaded from
https://github.com/dylanmei/docker-zeppelin

and dropped into the folder "data". Every file you leave there will be copied
to the path /data into the docker container.

To build the image and run the container only need these two commands being already where the Dockerfile is:

docker build -t zeppelin-docker .

docker run --rm -p 8080:8080 zeppelin-docker

spark example scripts are in the folder "examples/spark"

In order to use pyspark, just create a Spark Notebook and type %pyspark to use the pyspark interpreter
